...
const STATUS_OPTIONS = [
  { value: 'attesa', label: 'In Attesa', bg: '#fef9c3', text: '#a16207', border: '#fde047' },
  { value: 'accettato', label: 'Accettato', bg: '#dcfce7', text: '#166534', border: '#86efac' },
  { value: 'rifiutato', label: 'Rifiutato', bg: '#fee2e2', text: '#991b1b', border: '#fca5a5' },
]
...
export default function PreventiviPage() {
  ...
  const subtotale = (voci ?? []).reduce((s, v) => s + (Number(v?.quantita) || 0) * (Number(v?.prezzoUnitario) || 0), 0)
  ...
  const openEdit = (p: Preventivo) => {
    ...
    try {
      ...
      setVoci(parsed.map(v => ({ ...v, id: Math.random().toString(36).slice(2) })))
    } catch {
      setVoci([newVoce()])
    }
    setShowForm(true)
  ...
  const applyContatto = (id: string) => {
    ...
    if (c) setForm(f => ({ ...f, nomeCliente: c.nome ?? '', emailCliente: c.email ?? '' }))
  }
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      voci: voci.map(({ servizio, descrizione, quantita, prezzoUnitario }) => ({
        servizio, descrizione, quantita: Number(quantita), prezzoUnitario: Number(prezzoUnitario),
      })),
  ...
  const updateVoce = (id: string, field: keyof Omit<Voce, 'id'>, val: string | number) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          { label: 'In Attesa', val: fmt((preventivi ?? []).filter(p => p?.status === 'attesa').reduce((s, p) => s + (p?.totale ?? 0), 0)), color: 'text-amber-600' },
        ].map(k => (
        ...
        <table className="w-full min-w-[700px]">
          ...
          <tbody>
            ...
            {filtered.map(p => {
              ...
              const isScaduto = !['accettato'].includes(p.status) && p.dataScadenza && new Date(p.dataScadenza) < new Date()
              return (
                <tr key={p.id} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${isScaduto ? 'bg-red-50' : ''}`}>
                  <td className="px-4 py-3 text-sm font-mono font-medium text-gray-700">{p.numero ?? '—'}</td>
                  <td className="px-4 py-3">
                    <p className="text-sm font-medium text-gray-900">{p.nomeCliente ?? '—'}</p>
                    {p.aziendaCliente && <p className="text-xs text-gray-400">{p.aziendaCliente}</p>}
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600 max-w-[180px] truncate">{p.oggetto ?? ''}</td>
                  <td className="px-4 py-3 text-sm text-gray-500">
                    {new Date(p.createdAt).toLocaleDateString('it-IT')}
                  </td>
                  <td className="px-4 py-3 text-sm">
                    {p.dataScadenza ? (
                      <span className={`text-xs font-medium ${isScaduto ? 'text-red-600' : 'text-gray-500'}`}>
                  ...
                  <td className="px-4 py-3 text-sm font-semibold text-gray-900 text-right">{fmt(p.totale ?? 0)}</td>
                  <td className="px-4 py-3 text-center">
                    <button
                      onClick={() => {
                        const next = p.status === 'attesa' ? 'accettato' : p.status === 'accettato' ? 'rifiutato' : 'attesa'
                        quickStatus(p, next)
                      }}
                      className="inline-flex items-center gap-1 text-xs font-medium px-3 py-1 rounded-full border transition-colors"
                      style={{ background: st.bg, color: st.text, borderColor: st.border }}
                    >
                      {p.status === 'accettato' && <Check className="w-3 h-3" />}
          ...
          <div className="glass-modal rounded-t-2xl md:rounded-2xl w-full max-w-3xl md:my-4 p-4 md:p-6 space-y-4 md:space-y-5 max-h-[95vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                <label className="text-xs font-medium text-gray-600 block mb-1">Oggetto / Titolo *</label>
                <input
                  type="text"
                  value={form.oggetto}
                  ...
                  {STATUS_OPTIONS.map(o => {
                    ...
                    return (
                      <button
                        ...
                        style={active
                          ? { background: o.bg, color: o.text, borderColor: o.border }
                          : { background: '#fff', borderColor: '#e2e8f0', color: '#94a3b8' }}
                      >
                        {o.label}
                      </button>
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <input
                    ...
                    onChange={e => setForm(f => ({ ...f, aziendaCliente: e.target.value }))}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs font-medium text-gray-600">Voci / Servizi *</label>
                <button
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="text-xs font-semibold text-gray-500 px-3 py-2 text-left w-[32%]">Servizio</th>
                      <th className="text-xs font-semibold text-gray-500 px-3 py-2 text-left w-[30%]">Descrizione</th>
                      <th className="text-xs font-semibold text-gray-500 px-3 py-2 text-center w-[10%]">Q.tà</th>
                      <th className="text-xs font-semibold text-gray-500 px-3 py-2 text-right w-[14%]">€/Unit.</th>
                      <th className="text-xs font-semibold text-gray-500 px-3 py-2 text-right w-[12%]">Totale</th>
                      <th className="w-[2%]" />
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={v.id} className="border-t border-gray-100">
                        ...
                        <td className="px-2 py-1.5">
                          <input
                            type="text"
                            value={v.descrizione}
                            onChange={e => updateVoce(v.id, 'descrizione', e.target.value)}
                        ...
                        <td className="px-2 py-1.5">
                          <input
                            ...
                            min={1}
                            value={v.quantita}
                            onChange={e => updateVoce(v.id, 'quantita', parseFloat(e.target.value) || 1)}
                        ...
                        <td className="px-2 py-1.5">
                          <input
                            ...
                            onChange={e => updateVoce(v.id, 'prezzoUnitario', parseFloat(e.target.value) || 0)}
                        ...
                        <td className="px-1 py-1.5">
                          {voci.length > 1 && (
                            <button
                              onClick={() => setVoci(vs => vs.filter(x => x.id !== v.id))}
                  ...
                  <div className="flex justify-between items-center text-gray-600">
                    ...
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        max={100}
                        value={form.iva}
                        onChange={e => setForm(f => ({ ...f, iva: parseFloat(e.target.value) || 0 }))}
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-medium text-gray-600 block mb-1">Condizioni commerciali</label>
                <textarea
                  value={form.condizioni}
                  onChange={e => setForm(f => ({ ...f, condizioni: e.target.value }))}
                  rows={4}
