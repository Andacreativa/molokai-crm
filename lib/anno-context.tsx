...
interface AnnoCtx { anno: number; setAnno: (a: number) => void }

const AnnoContext = createContext<AnnoCtx>({ anno: ANNI[0], setAnno: () => {} })
...
export function AnnoProvider({ children }: { children: ReactNode }) {
  ...
  return <AnnoContext.Provider value={{ anno, setAnno }}>{children}</AnnoContext.Provider>
}
