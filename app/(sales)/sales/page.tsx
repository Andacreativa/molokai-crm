...
export default function SalesDashboard() {
  ...
  const saveLead = async () => {
    ...
    await fetch("/api/leads", {
      ...
      body: JSON.stringify({
        ...form,
        valore: form.valore ? Number(form.valore) : null,
      }),
    });
    setShowForm(false);
  ...
  const totaleVinto = leads
    .filter((l) => l.stage === "vinto")
    .reduce((s, l) => s + (l.valore ?? 0), 0);
  const totalePipeline = leads
    ...
    <div className="flex flex-col h-screen overflow-hidden -m-4 -mt-14 md:-m-8">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 pl-14 pr-4 md:px-8 py-3 md:py-5 border-b border-gray-200 bg-white/60 backdrop-blur shrink-0">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          >
            ...
            <span className="hidden sm:inline">Nuovo</span> Lead
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-x-auto overflow-y-hidden">
          <div
            className="flex gap-3 md:gap-4 h-full px-4 md:px-8 py-4 md:py-6"
          ...
          >
            {STAGES.map((stage) => {
              ...
              return (
                <div
                  key={stage.key}
                  className="flex flex-col w-52 md:w-64 shrink-0"
                >
                  {/* Column header */}
          ...
          <div className="fixed inset-0 z-40 md:relative md:inset-auto md:w-80 md:shrink-0 border-l border-gray-200 bg-white/95 md:bg-white/80 backdrop-blur flex flex-col overflow-hidden">
            ...
            <div className="px-5 py-4 border-b border-gray-100 shrink-0">
              <div className="flex items-start justify-between">
                ...
                <button
                  ...
                  className="p-1 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 shrink-0 ml-2"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            {selected.preventivi?.length > 0 && (
              <div className="px-5 py-3 border-b border-gray-100 shrink-0 space-y-2">
                ...
                {selected.preventivi.map((p) => {
                  const guadagno =
                    p.subtotale * (1 - (p.feeCommerciale ?? 0) / 100);
                  const statusColor =
                    p.status === "accettato"
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between gap-1">
                        ...
                        <span
                          className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full border ${statusColor}`}
                        >
                          {p.status === "accettato"
                            ? "Accettato"
                            : p.status === "rifiutato"
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              ...
              {selected.appunti.length === 0 && (
                <p className="text-xs text-gray-400 text-center py-6">
                  ...
                  <br />
                  Aggiungi note dalla call qui sotto.
                </p>
              )}
              {[...selected.appunti].reverse().map((a) => (
                <div
                  key={a.id}
                  className="group relative bg-amber-50 border border-amber-100 rounded-xl px-3.5 py-3"
                >
            ...
            <div className="px-5 py-4 border-t border-gray-100 shrink-0">
              ...
              <button
                ...
                style={{ background: BRAND }}
                onMouseEnter={(e) => {
                  if (!sendingAppunto && newAppunto.trim())
                    (e.currentTarget as HTMLElement).style.opacity = "0.85";
                }}
                onMouseLeave={(e) =>
                  ((e.currentTarget as HTMLElement).style.opacity = "1")
            ...
            <span className="text-gray-400">
              ...
              <span className="font-bold text-emerald-600">
                {fmt(
                  preventivi.reduce(
                    (s, p) =>
                      s + p.subtotale * (1 - (p.feeCommerciale ?? 0) / 100),
                    0,
                  ),
                )}
              </span>
            ...
            <thead>
              <tr className="border-b border-gray-100">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left font-semibold text-gray-400 uppercase tracking-wide px-3 py-2 whitespace-nowrap last:text-right"
                  >
                    {h}
                  </th>
                ))}
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      ...
                      {p.aziendaCliente && (
                        <p className="text-gray-400">{p.aziendaCliente}</p>
                      )}
                    </td>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    value={form.azienda}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, azienda: e.target.value }))
                    }
